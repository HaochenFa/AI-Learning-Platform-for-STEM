  ...
  return (
    <SheetPortal>
      ...
      <DialogPrimitive.Content
        className={cn(
          "fixed z-50 gap-4 border border-default bg-white p-6 shadow-lg transition ease-in-out",
...
function SheetTitle({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Title>) {
  return <DialogPrimitive.Title className={cn("text-lg font-semibold text-ui-primary", className)} {...props} />;
}

function SheetDescription({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Description>) {
  return <DialogPrimitive.Description className={cn("text-sm text-ui-muted", className)} {...props} />;
}
